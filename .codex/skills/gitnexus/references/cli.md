...
Run from the project root. This parses all source files, builds the knowledge graph, writes it to `.gitnexus/`, and generates agent context files. In this repository GitNexus is configured for Codex, so keep `AGENTS.md` and `.codex/skills/gitnexus/`.
...
**When to run:** First time in a project, after major code changes, or when `gitnexus://repo/{name}/context` reports the index is stale. Codex should run `analyze` explicitly when needed instead of relying on Claude-specific hooks.
...
- **Index is stale after re-analyzing**: Restart the Codex session or MCP server to reload the index
